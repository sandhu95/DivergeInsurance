...
app.get('/', (req, res) => {
  ...
  app.get('/showmessage', (req, res) => {
    db.collection('message').find().toArray()
      .then(results => {
        res.render('showmessage.ejs', { message: results })
      })
      .catch(/* ... */)
  })
...
app.delete('/deletemessage/:id',(req,res) =>{
  var id = req.params.id
  var myobj = { "_id": objectID(id) };
  db.collection("message").deleteOne(myobj, function(err, result) {
    if (err) throw err;
    console.log("1 document deleted in message collection");
    res.status(200);
    res.redirect("/showmessage");
    });
})